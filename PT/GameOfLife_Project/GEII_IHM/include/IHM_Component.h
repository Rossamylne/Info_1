...
typedef struct IHM_Component {
    ...
    void (*onMouseButtonDown)(struct IHM_Component *self, int localX, int localY);
    void (*onMouseButtonUp)(struct IHM_Component *self, int localX, int localY);
 ...
 * Si c'est le cas et que le pointeur de fonction onMouseButtonDown est valide, la fonction onMouseButtonDown() est invoquée.
 * @param self Pointeur d'auto-référence du composant concerné.
