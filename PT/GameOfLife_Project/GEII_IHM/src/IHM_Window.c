...
void IHM_Window_mouseButtonDown(IHM_Window_t *self, int x, int y, int bouton)
{
    ...
    while (tmpPtr != NULL)
    {
        if (!IHM_Component_mouseDown(tmpPtr->composant, x, y))
            break;
        tmpPtr = tmpPtr->suivant;
    }
    if (self->onMouseButtonDown != NULL)
        self->onMouseButtonDown(self, x, y, bouton);
}
...
void IHM_Window_mouseButtonUp(IHM_Window_t *self, int x, int y, int bouton)
{
    ...
    while (tmpPtr != NULL)
    {
        if (!IHM_Component_mouseUp(tmpPtr->composant, x, y))
            break;
        tmpPtr = tmpPtr->suivant;
    }
    if (self->onMouseButtonUp != NULL)
        self->onMouseButtonUp(self, x, y, bouton);
}
